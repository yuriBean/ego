...
export default function GameManagement() {
    ...
    const { id } = useParams();
    const [errors, setErrors] = useState({}); // To track validation errors for each option
    const temp = {
      value: "",
    };
    ...
    useEffect(() => {
      if (id) {
        try {
          axios
            .get(
              `${process.env.REACT_APP_BACKEND_PORT}/game/update?pageId=${id}`,
              {
                headers: {
            ...
            .then((res) => {
              ...
              const fetchedTotal = Object.keys(fetchedData.options).reduce(
                (acc, key) => {
    ...
    return (
      <div className="flex mb-20">
        ...
        <div className="flex flex-col items-center w-full mt-4 max-md:mt-10 max-md:max-w-full">
          ...
          <div className="flex justify-between gap-5 mt-12 text-xl font-semibold tracking-tight text-blue-950 max-md:mt-10">
            <div
              onClick={() => {
                setSelectedRow(1);
              }}
              className={`cursor-pointer text-center justify-center px-5 py-2.5  ${
            ...
            <div
              ...
              className={`cursor-pointer text-center justify-center px-5 py-2.5 whitespace-nowrap ${
                selectedRow === 2 ? "bg-indigo-400 text-white" : "text-black"
              } rounded-3xl`}
            >
              Design
            </div>
                ...
                <div className="flex flex-col my-5 w-full max-w-[1050px] max-md:max-w-full">
                  <div className="flex flex-col p-5 font-medium text-black bg-white leading-[140%] max-md:max-w-full mr-2 md:mr-5 shadow-[0px_5px_10px_1px_rgba(0,0,0,0.3)]">
                    <div className="flex gap-5 max-md:flex-col max-md:gap-0">
                      <div className="w-full mt-4 text-base font-medium leading-5 text-black">
                        Nom du Restaurant
                        <input
                          onChange={(e) =>
                            setGameFormate({
                ...
                <div className="flex flex-col p-5 font-medium text-black bg-white leading-[140%] max-md:max-w-full mr-2 md:mr-5 shadow-[0px_5px_10px_1px_rgba(0,0,0,0.3)]">
                  ...
                  <div className="flex flex-col items-start pr-20 mt-3.5 text-base max-md:pr-5 max-md:max-w-full ">
                    ...
                    >
                      ...
                      />
                      <label
                        htmlFor="enterBrandName"
                        id="enterBrandName"
                        className="my-auto"
                      >
                        Entrer le nom de la marque
                      </label>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col mt-9 w-full max-w-[1050px] max-md:max-w-full">
                  <div className="flex flex-col p-5 font-medium text-black bg-white leading-[140%] max-md:max-w-full mr-2 md:mr-5 shadow-[0px_5px_10px_1px_rgba(0,0,0,0.3)]">
                    ...
                    <div className="flex flex-col mt-3.5 max-md:max-w-full">
                      {data.map((Item, index) => (
                        <div
                          className="mt-4 text-base font-medium leading-5 text-black max-md:max-w-full"
                          key={index}
                        >
                          Option {index + 1}
                          <div className="flex items-start">
                            <input
                              ...
                              className={`flex flex-col justify-center px-3.5 py-2.5 mt-3 text-base leading-6 bg-white rounded-lg border ${
                                errors[`option${index + 1}`]
                                  ? "border-red-500"
                                  : "border-gray-300"
                              } border-solid shadow-sm text-zinc-400 max-md:max-w-full outline-none w-full`}
                              type="text"
                            ...
                            <div className="flex flex-col gap-2 ">
                              <select
                                onChange={(e) => {
                                  ...
                                  const previousValue =
                                    gameFormat.options[
                                      `option${index + 1}frequency`
                                    ] || 0;

                                  // Calculate new total
                                  const newTotal =
                                    total + previousValue - selectedValue;
                                ...
                                className={`md:ml-2 flex flex-col justify-center px-3.5 py-2.5 mt-3 text-base leading-6 bg-white rounded-lg border ${
                                  errors[`option${index + 1}`]
                                    ? "border-red-500"
                                    : "border-gray-300"
                                } border-solid shadow-sm text-zinc-400 max-md:max-w-full outline-none w-[100px]`}
                                value={
                                  gameFormat.options[
                                    `option${index + 1}frequency`
                                  ] || ""
                    ...
                    <div
                      onClick={() => {
                        ...
                        data.forEach((_, index) => {
                          const textValue =
                            gameFormat.options[`option${index + 1}`];
                          const dropdownValue =
                            gameFormat.options[`option${index + 1}frequency`];

                          if (textValue && !dropdownValue) {
                            newErrors[`option${index + 1}`] = `select a value `;
                          }
                        });
            ...
            {selectedRow === 2 && (
              <>
                <div className="flex flex-wrap p-5 font-medium text-black bg-white leading-[140%] shadow-[0px_5px_10px_1px_rgba(0,0,0,0.3)]">
                  <div className="w-full mb-4 text-2xl max-md:max-w-full">
                    Couleur de la Roue
                  </div>
                ...
                <div className="flex flex-wrap p-5 font-medium text-black bg-white leading-[140%] shadow-[0px_5px_10px_1px_rgba(0,0,0,0.3)] mt-7">
                  <div className="mb-4 text-2xl max-md:max-w-full">
                    Couleur du Bouton
                  </div>
                  <div className="flex gap-[2vw] flex-wrap">
            ...
            {selectedRow === 3 && (
              <>
                  ...
                  <div className="flex flex-col p-5 font-medium text-black bg-white leading-[140%] max-md:max-w-full mr-2 md:mr-5 shadow-[0px_5px_10px_1px_rgba(0,0,0,0.3)]">
                    ...
                    <div className="flex flex-col mt-3.5 max-md:max-w-full">
                      <div className="mt-4 text-base font-medium leading-5 text-black max-md:max-w-full">
                        Contenu
                        <p
                          className="justify-center px-3.5 py-2.5 mt-3 text-base bg-white shadow-sm text-zinc-400 max-md:max-w-full outline-none w-full"
                          type="text"
                        >
                          Bravo, vous avez gagné {gameFormat.resturantName} !
                          Pour bénéficier pleinement de votre cadeau, nous vous
                          invitons à vous rendre à l'adresse suivante, à la date
                          de votre choix, et de présenter ce mail :
                          <span>
                            <br />
                            <br />
                          </span>
                          ...
                          Attention, votre cadeau n'est disponible que pendant
                          une durée limitée de 10 jours à partir de demain ! Ne
                          laissez pas cette occasion exceptionnelle vous
                          échapper, saisissez-la dès maintenant et profitez-en
                          au maximum ! Un minimum d’achat de 10 € est requis
                          ...
                          <span>
                            <strong>
                              Ceci est un message automatisé, veuillez ne pas
                              répondre.
                            </strong>
                          </span>
                        </p>
                ...
                <div className="flex max-w-[1050px] ml-5 mr-4 mt-5 justify-end max-md:ml-0 max-md:w-full">
                  ...
                  >
                    <label className="cursor-pointer">{`${
                      id ? "Mettre à jour page" : "Créer page destination"
                    }`}</label>
                  </div>
                </div>
              </>
