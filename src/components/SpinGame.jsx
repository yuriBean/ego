...
const SpinGame = () => {
  ...
  useEffect(() => {
    if (id) {
      try {
        axios
          .get(`${process.env.REACT_APP_BACKEND_PORT}/game/play?pageId=${id}`, {
            headers: {
  ...
  useEffect(() => {
    if (gameFormat.buttonColor === "#E3F2FD") setTextColor("text-black");
  ...
  const spinedClick = (gameFormat) => {
    if (email) {
      axios
        .post(`${process.env.REACT_APP_BACKEND_PORT}/review/check`, { email })
        .then((res) => {
          setIsSpined(true);

          // Extract options and frequencies
          const frequencies = [
            {
              angle: 0,
              frequency: gameFormat.options.option6frequency,
              label: "try again",
            },
            {
              angle: 45,
              frequency: gameFormat.options.option5frequency,
              label: 5,
            },
            {
              angle: 90,
              frequency: gameFormat.options.option4frequency,
              label: 4,
            },
            {
              angle: 135,
              frequency: gameFormat.options.option7frequency,
              label: "try again",
            },
            {
              angle: 180,
              frequency: gameFormat.options.option3frequency,
              label: 3,
            },
            {
              angle: 225,
              frequency: gameFormat.options.option2frequency,
              label: 2,
            },
            {
              angle: 270,
              frequency: gameFormat.options.option8frequency,
              label: "try again",
            },
            {
              angle: 315,
              frequency: gameFormat.options.option1frequency,
              label: 1,
            },
          ];

          // Filter out null or invalid frequencies
          const validOptions = frequencies.filter(
            (option) => option.frequency !== null && option.frequency > 0
          );

          // Create a Picker instance
          const picker = new Picker();

          // Add valid options to the picker
          validOptions.forEach((option) => {
            picker.option(option.angle, option.frequency);
          });
          ...
          const selected = validOptions.find(
            (option) => option.angle === selectedOption
          )?.label;
  ...
  const sendEmail = () => {
    if (selectedIndex !== -1 && selectedIndex !== "try again" && email) {
      try {
        axios
          .post(
            `${process.env.REACT_APP_BACKEND_PORT}/result`,
            {
              email: email,
        ...
        <div>
          <div className="flex justify-center mt-3">
            {gameFormat.logo ? (
              ...
              <h1 className="font-sans text-2xl italic font-bold">
                {gameFormat?.brandName}
              </h1>
            )}
          </div>
          <div className="flex flex-col md:flex-row gap-11 max-w-[900px] justify-between mx-auto items-center mt-10">
            ...
            >
              ...
              <div className="wheel">
                <div
                  ...
                  style={{
                    "--i": 1,
                    "--clr": gameFormat?.wheelColorPair?.color1
                      ? gameFormat?.wheelColorPair?.color1
                      : "#8497FC",
                  }}
                >
                ...
                <div
                  ...
                  style={{
                    "--i": 2,
                    "--clr": gameFormat?.wheelColorPair?.color2
                      ? gameFormat?.wheelColorPair?.color2
                      : "#FDFDAF",
                  }}
                >
            ...
            <div className="flex flex-col font-medium text-black leading-[140%] max-w-[454px]">
              ...
              <div
                ...
                style={{
                  backgroundColor: gameFormat?.buttonColor
                    ? gameFormat?.buttonColor
                    : "#4F46E5",
                }}
              >
          ...
          <div className="flex justify-center gap-4">
            <a
              onClick={() => {
                const data = {
                  ...gameFormat,
                  instagramClicks: gameFormat.instagramClicks + 1,
                };

                updateGame(data);
              }}
              href={gameFormat?.instagram}
            >
              <img
                loading="lazy"
            ...
            <a
              onClick={() => {
                const data = {
                  ...gameFormat,
                  facebookClicks: gameFormat.facebookClicks + 1,
                };

                updateGame(data);
              }}
              href={gameFormat?.facebook}
            >
              <img
                loading="lazy"
            ...
            <a
              onClick={() => {
                const data = {
                  ...gameFormat,
                  twitterClicks: gameFormat.twitterClicks + 1,
                };

                updateGame(data);
              }}
              href={gameFormat?.twitter}
            >
              <img
                loading="lazy"
            ...
            <a
              onClick={() => {
                const data = {
                  ...gameFormat,
                  googleMapsClicks: gameFormat.googleMapsClicks + 1,
                };

                updateGame(data);
              }}
              href={gameFormat?.googleMaps}
            >
              <img
                loading="lazy"
          ...
          <div className="flex flex-col items-center justify-center mt-28">
            ...
            />
            <div className="flex justify-center mb-5 text-sm text-slate-800">
          ...
          <ReviewModel
            open={reviewModel}
            setOpen={setReviewModel}
            gameFormat={gameFormat}
            sendEmail={sendEmail}
          />
          <ReviewModel2
            reviewModel2={reviewModel2}
            setReviewModel2={setReviewModel2}
            url={url}
            followOrReview={gameFormat.followOrReview}
          />
        </div>
      )}
      {openMessage && (
